...
int vncSockConnect::Init(vncServer *server, UINT port)
{
	// Save the port id
	m_port = port;
	...
	// Bind it
	int res = m_socket.Bind(m_port, server->LoopbackOnly()); //#here
	...
	if (((vncSockConnectThread *)m_thread)->Init(&m_socket, server))
		res = CONNECTION_SUCCESSFUL;
	else
		res = ERROR_CONNECTING_TO_PROXY;
	return res;
}
