...
VSocket::VSocket()
:	m_uiBindedLoopbackPortNumber(0)
{
...
//extern bool RemoveProxyObject(UINT uiPort );
VSocket::~VSocket()
{
//	if (m_uiBindedLoopbackPortNumber)
//	{
//		RemoveProxyObject(m_uiBindedLoopbackPortNumber);
//	}
...
bool VSocket::GetPort( UINT& rSocketPort)
{
	SOCKADDR_IN sockAddr;
	memset(&sockAddr, 0, sizeof(sockAddr));

	int nSockAddrLen = sizeof(sockAddr);
	if (getsockname( sock, (SOCKADDR*)&sockAddr, &nSockAddrLen) != SOCKET_ERROR)
	{
...
extern int InitLoopbackListner( UINT uiPort );

int VSocket::Bind(const VCard port, const VBool localOnly)
{
	...
	else
	{
		...
		{
			UINT uiPort;
			if (GetEchowareUsing() && GetPort( uiPort ))
			{
				int res = InitLoopbackListner( uiPort ); //#here : create and show the Startup dialog inside
				if (res == CONNECTION_SUCCESSFUL)
					m_uiBindedLoopbackPortNumber = uiPort;
				return res;
			}
		}
	}
