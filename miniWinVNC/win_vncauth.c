...
unsigned char WinVncfixedkey[8] = {23,82,107,6,35,78,88,7};


// Encrypt a password and store it in a file.
int WinVncEncryptPasswd(char *passwd, char *encryptedPasswd)
{
...
char* WinVncDecryptPasswd(char *inouttext)
{
    unsigned char *passwd = (unsigned char *)malloc(17);
    memset(passwd, 0, 17);

    deskey(WinVncfixedkey, DE1);
...
char* WinVncDecryptPasswdMs(char *inouttext) 
{ 
...
void WinVncRandomBytes(unsigned char *where) 
{
	int i;
...
void WinVncRandomBytesMs(unsigned char *where) 
{
	int i;
	static unsigned int seed;
	seed += (unsigned int) time(0) + _getpid() + _getpid() * 987654;
...
void WinVncEncryptBytes(unsigned char *where, const char *passwd)
{
    unsigned char key[8];
    unsigned int i;
...
void WinVncDecryptBytes(unsigned char *where, const int length, const unsigned char *key) 
{
	int i, j;
