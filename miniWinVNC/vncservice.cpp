...
void mytest() {
   ...
   if (hlib)  
	   WTSGetActiveConsoleSessionIdF=(pWTSGetActiveConsoleSessionId)GetProcAddress(hlib, "WTSGetActiveConsoleSessionId"); 
   if (WTSGetActiveConsoleSessionIdF!=NULL)
	   dwSessionId =WTSGetActiveConsoleSessionIdF();
   vnclog.Print(LL_INTERR, VNCLOG("dwSessionId %d"), dwSessionId);
   }
   ...
   __try {
      ...
      if (!OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY 
         ...
         vnclog.Print(LL_INTERR, VNCLOG("OpenProcessToken %d"), GetLastError());
		 RevertToSelf();
         __leave;
      }
      ...
      if (!LookupPrivilegeValue(NULL, SE_TCB_NAME, 
		...
		vnclog.Print(LL_INTERR, VNCLOG("LookupPrivilegeValue %d"), GetLastError());
         __leave;
      }
      ...
      if (!AdjustTokenPrivileges(hToken, FALSE, &tp, 0, NULL, 0)
         ...
         vnclog.Print(LL_INTERR, VNCLOG("AdjustTokenPrivileges %d"), GetLastError());
         __leave;
      }

      // Set the token's session ID.
      if (!SetTokenInformation(hToken, TokenSessionId, &dwSessionId, 
            sizeof(DWORD))) {

         vnclog.Print(LL_INTERR, VNCLOG("SetTokenInformation %d"), GetLastError());
         __leave;
      }
	  ...
	  for (int i=0; i<10;i++)
	  {
	     ...
	     strcat_s(szCurrentDir, _MAX_PATH," -fus");
		 if (!CreateProcessAsUser(hToken, NULL, szCurrentDir, NULL, NULL, FALSE, NORMAL_PRIORITY_CLASS, NULL, NULL, &si, &pi))
			{
			    if ( GetLastError()!=2) __leave;
				else vnclog.Print(LL_INTERR, VNCLOG("Retry, give session time to startup"));
				vnclog.Print(LL_INTERR, VNCLOG("CreateProcessAsUser %d"), GetLastError());
				Sleep(2000);
			}
		else break;
	  }
...
void LogErrorMsg(char *message)
{
	...
	_snprintf_s(msgbuff, 256, 256, "%s error: %d", VNCSERVICENAME, g_error);
    strings[0] = msgbuff;
    strings[1] = message;
