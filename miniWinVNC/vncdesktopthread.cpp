...
vncDesktopThread::copy_bitmaps_to_buffer(ULONG i, rfb::Region2D &rgncache, rfb::UpdateTracker &tracker)
{
		...
		int w = m_desktop->m_videodriver->bufdata.buffer->pointrect[i].rect.right - 
			m_desktop->m_videodriver->bufdata.buffer->pointrect[i].rect.left;
		int y = m_desktop->m_videodriver->bufdata.buffer->pointrect[i].rect.top;
		int h = m_desktop->m_videodriver->bufdata.buffer->pointrect[i].rect.bottom - 
			m_desktop->m_videodriver->bufdata.buffer->pointrect[i].rect.top;

		if (!ClipRect(&x, &y, &w, &h, m_desktop->m_bmrect.tl.x, m_desktop->m_bmrect.tl.y,
			m_desktop->m_bmrect.br.x - m_desktop->m_bmrect.tl.x, 
			m_desktop->m_bmrect.br.y - m_desktop->m_bmrect.tl.y)) return;
		...
		switch(m_desktop->m_videodriver->bufdata.buffer->pointrect[i].type)
			{
				case SCREEN_SCREEN:
					{
						int dx = m_desktop->m_videodriver->bufdata.buffer->pointrect[i].point.x;
						int dy = m_desktop->m_videodriver->bufdata.buffer->pointrect[i].point.y;
...
vncDesktopThread::handle_driver_changes(rfb::Region2D &rgncache, rfb::UpdateTracker &tracker)
{
	...
	int oldCounter = m_desktop->m_videodriver->oldCounter;
	int counter = m_desktop->m_videodriver->bufdata.buffer->counter;

	//m_desktop->m_videodriver->bufdata.buffer->pointrect[0].type=0;

	if (oldCounter == counter) return FALSE;
	if (counter < 1 || counter > MAXCHANGES_BUF) return FALSE;
	...
	{
		for (ULONG i = (ULONG) oldCounter; i < (ULONG) counter; i++)
	...
	else
	{
		...
		for (i = oldCounter; i < MAXCHANGES_BUF; i++)
			copy_bitmaps_to_buffer(i, rgncache, tracker);
		for (i = 0; i < (ULONG) counter; i++)
			copy_bitmaps_to_buffer(i, rgncache, tracker);
	}	
	m_desktop->m_videodriver->oldCounter = (ULONG) counter;
...
//   if (!vncService::RunningAsService()) return;
   ...
   if (WTSGetActiveConsoleSessionIdF)
	   dwSessionId = WTSGetActiveConsoleSessionIdF();
   if (ProcessIdToSessionIdF)
		ProcessIdToSessionIdF(GetCurrentProcessId(), &dwCurrentSessionId);

   if (dwSessionId != dwCurrentSessionId)
   {
		//Black screen only session 0 can be handled from a service
		if (WinStationConnectF)
		{
			if (WinStationConnectF(0, dwCurrentSessionId, dwSessionId, L"", 0))
			{
				if (LockWorkStationF)
				{
					LockWorkStationF();
...
*/
	...
	XRichCursorEnabled = m_desktop->m_server->IsXRichCursorEnabled() != FALSE;
	// no video driver
/*
...
//			m_server->SetSWOffset(m_desktop->m_SWOffsetx,m_desktop->m_SWOffsety);
			...
			if (m_desktop->m_server->UpdateWanted())
			{
				...
				if (!m_desktop->m_hookdriver)
				{
					...
					{
						if (lTime - m_lLastUpdateTime > 300)
						{
...
*/					}
					...
					if (!m_desktop->m_hookdriver)// If several clients, send them all the mouse updates
					{
						// Render the mouse
