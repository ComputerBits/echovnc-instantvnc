...
// Constants
const UINT RFB_SCREEN_UPDATE = RegisterWindowMessage("EchoVNC.Update.DrawRect");
const UINT RFB_COPYRECT_UPDATE = RegisterWindowMessage("EchoVNC.Update.CopyRect");
const UINT RFB_MOUSE_UPDATE = RegisterWindowMessage("EchoVNC.Update.Mouse");
const char szDesktopSink[] = "EchoVNC desktop sink";
...
vncDesktop::FastDetectChanges(rfb::Region2D &rgn, rfb::Rect &rect, int nZone, bool fTurbo)
{
	...
	if (m_lGridsList.empty())
	{
		for (int i = 0; i < (5 * PIXEL_BLOCK_SIZE / GRID_OFFSET); i++)
		{
			...
			if (pList != NULL)
			{
			    ...
			    vnclog.Print(LL_INTINFO, VNCLOG("### PixelsGrid %d created!"), i);
			}
		}
	}
	...
	int size = ScreenBuffSize();
	BYTE *buff = NULL;

	if (vncService::VersionMajor() == 6)
	{
	// no video driver
/*		if (m_videodriver && m_server->Driver() && m_videodriver->IsActive())
		{
			buff = m_videodriver->GetScreenView();
		}
		else*/
		{
			buff = (BYTE*)m_DIBbits;
	...
	BYTE *buffpos = NULL;
	BOOL bufNULL = buff == NULL;
	if (bufNULL)
		vnclog.Print(LL_INTINFO, VNCLOG("FastDetectChanges: bufNULL == TRUE"));
	...
	for (y = rect.tl.y; y < rect.br.y; y += PIXEL_BLOCK_SIZE)
	{
		...
		for (x = rect.tl.x; x < rect.br.x; x += PIXEL_BLOCK_SIZE)
		{
			...
			{
				COLORREF PixelColor;
				if (bufNULL || (vncService::VersionMajor() < 6))
				{
					PixelColor = GetPixel(m_hrootdc, xo, yo);
				}
				else
				{
					if (yo >= rect.br.y)
				   ...
				   buffpos = buff + (m_bytesPerRow * yo) + (bytesPerPixel * xo);
				   PixelColor = *buffpos;
				}
...
vncDesktop::vncDesktop()
{
	...
	char szCurrentDir[_MAX_PATH];
		if (GetModuleFileName(NULL, szCurrentDir, _MAX_PATH))
		{
			...
			if (p == NULL) return;
			*p = '\0';
			strcat_s(szCurrentDir,_MAX_PATH,"\\vnchooks.dll");
		}
	UnSetHooks=NULL;
...
BOOL
vncDesktop::Startup(bool display_changed)
{
	// Initialise the Desktop object

	if (!InitDesktop())
	{
		vnclog.Print(LL_INTINFO, "vncDesktop::Startup: InitDesktop Failed err = %d", GetLastError());
		if (vncService::VersionMajor() < 6)
		{
			return FALSE;
		}
...
BOOL
vncDesktop::Shutdown(bool display_changed)
{
...
BOOL CALLBACK EnumWindowsHnd(HWND hwnd, LPARAM arg)
{
 ...
 strcpy_s(szNameAppli, 16, (LPSTR)(((vncDesktop*)arg)->GetServerPointer()->GetWindowName()));
 szNameAppli[15]=0;
								...
								if (DriverFound)
									{
										...
										if (m_hrootdc) DriverType=MIRAGE;
										Checkmonitors();
...
*/			}//OS
	...
	if (m_nEnableDualMonitors)
	{
		if (OSVersion() != 3 && OSVersion() != 5)
		{
			m_bmrect = rfb::Rect(GetSystemMetrics(SM_XVIRTUALSCREEN), GetSystemMetrics(SM_YVIRTUALSCREEN), 
								 GetSystemMetrics(SM_XVIRTUALSCREEN) + GetSystemMetrics(SM_CXVIRTUALSCREEN),
								 GetSystemMetrics(SM_YVIRTUALSCREEN) + GetSystemMetrics(SM_CYVIRTUALSCREEN));
		}
		else
		{
			m_bmrect = rfb::Rect(0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN));
		}
	}
	else
	{
	...
	if ((GetDeviceCaps(m_hrootdc, RASTERCAPS) & RC_BITBLT) == 0)
	{
		MessageBox(
			...
			"EchoVNC cannot be used with this graphic device driver",
			szAppName,
			MB_ICONSTOP | MB_OK
			);
		return FALSE;
	...
	if ((GetDeviceCaps(m_hmemdc, RASTERCAPS) & RC_DI_BITMAP) == 0)
	{
		MessageBox(
			...
			"EchoVNC cannot be used with this graphics device driver",
			szAppName,
			MB_ICONSTOP | MB_OK
			);
		return FALSE;
...
void
vncDesktop::CaptureScreenFromVideoDriver(const rfb::Rect &rect, BYTE *scrBuff, const BYTE *srcbuffpos)
{
	assert(rect.enclosed_by(m_bmrect));
	if (!m_videodriver) return;
	if (!m_server->Driver()) return;
	if (!m_videodriver->IsActive()) return;

	const int crect_re_vd_left = rect.tl.x - m_bmrect.tl.x;
	const int crect_re_vd_top = rect.tl.y - m_bmrect.tl.y;

	if (crect_re_vd_left < 0 || crect_re_vd_top < 0) return;
	...
	BYTE *destbuffpos = scrBuff + bmoffset;
	srcbuffpos += bmoffset;
...
/*	if (m_videodriver && m_server->Driver() && m_videodriver->IsActive())
	{
		CaptureScreenFromVideoDriver(rect, scrBuff, m_videodriver->GetScreenView());
		return;
	}*/
	...
	BOOL blitok = BitBlt(m_hmemdc, rect.tl.x, rect.tl.y, (rect.br.x - rect.tl.x), (rect.br.y - rect.tl.y),
...
void
vncDesktop::CaptureMouseFromVideoDriver(const rfb::Rect &rect, BYTE *scrBuff, const BYTE *srcbuffpos)
{
}
...
/*	if (m_videodriver && m_server->Driver() && m_videodriver->IsActive())
	{
		CaptureMouseFromVideoDriver(m_cursorpos, scrBuff, m_videodriver->GetScreenView());
		return;
	}*/
...
DesktopWndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COPYDATA:
			...
			if (_this->pMyCDS->dwData==112233)
			{
					...
					char *myptr;
					char split[4][6];
					strcpy_s(mytext,1024,(LPCSTR)_this->pMyCDS->lpData);
					myptr=mytext;
					for (int j =0; j<(int) (mysize/20);j++)
					{
						for (int i=0;i<4;i++)
							{
								strcpy_s(split[i],6,"     ");
								strncpy_s(split[i],6,myptr,4);
								myptr=myptr+5;
							}
	...
	case WM_DISPLAYCHANGE:
		...
		// For a temp resolution we don't use the driver, to fix the mirage driver
...
vncDesktop::SetDisableInput(bool enabled)
{
	...
	if (enabled)
	{
		...
		else
		{
			...
			OldCaptureBlending=m_fCaptureAlphaBlending != FALSE;
...
BOOL vncDesktop::VideoBuffer()
{
	// no video driver
	/*
	if (!m_videodriver) return FALSE;
	// If we reach this place, the driver was active
	if (m_videodriver->IsActive()) return TRUE;
...
/*	if (!RestartDriver)
	...
	if (m_videodriver->IsMirageDriverActive())
	{
		m_videodriver->DeActivate();
	}
	if (!m_videodriver->IsActive())
	{
		Checkmonitors();
		...
		if (nr_monitors==1)
		{
			m_ScreenOffsetx=mymonitor[0].offsetx;
			m_ScreenOffsety=mymonitor[0].offsety;
			m_videodriver->Activate(true,mymonitor[0].offsetx,mymonitor[0].offsety,mymonitor[0].Width,mymonitor[0].Height);
		}
		if (nr_monitors>1)
		{
			m_ScreenOffsetx=mymonitor[2].offsetx;
			m_ScreenOffsety=mymonitor[2].offsety;
			m_videodriver->Activate(true,mymonitor[2].offsetx,mymonitor[2].offsety,mymonitor[2].Width,mymonitor[2].Height);
		}

		vnclog.Print(LL_INTERR, VNCLOG("Start Mirage driver\n"));
		if (m_videodriver->MapSharedbuffers(true))
		{
	...
	if (!m_videodriver->IsActive()) 
	{
		vnclog.Print(LL_INTERR, VNCLOG("Start Mirage driver Failed"));
		vnclog.Print(LL_INTERR, VNCLOG("Using non driver mode"));
		if (m_videodriver) delete m_videodriver;
		m_videodriver = NULL;
...
void vncDesktop::SethookMechanism(BOOL hookall,BOOL hookdriver)
{
	...
	else if(OSVersion()==1)
	{
		...
		vnclog.Print(LL_INTERR, VNCLOG("Sethook_restart_wanted hook=%d driver=%d"),m_hookdll,m_hookdriver);
		if (Hookdll_Changed)
...
void vncDesktop::StartStopddihook(BOOL enabled)
{
	if (enabled)
	{
		...
		if (GetModuleFileName(NULL, szCurrentDir, _MAX_PATH))
		{
			...
			if (p == NULL) return;
			*p = '\0';
			strcat_s(szCurrentDir,_MAX_PATH,"\\16bithlp.exe");
		}
		// Add ddi hook
...
{
	// no video driver
	SethookMechanism(m_server->Hook(), /*m_server->Driver()*/ FALSE);
}
