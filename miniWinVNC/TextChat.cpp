...
TextChat::~TextChat()
{
	...
	m_fTextChatRunning = false;
	SendMessage(m_hDlg, WM_COMMAND, IDCANCEL, 0L);
	...
	, long nSize /*= 0x75*/, const char* szFaceName /*= "MS Sans Serif"*/, DWORD dwColor /*= BLACK*/)
{
	...
	{		
		...
		strcpy_s( cf.szFaceName, LF_FACESIZE, szFaceName);
...
void TextChat::PrintMessage(const char* szMessage,const char* szSender,DWORD dwColor /*= BLACK*/)
{
	...
	if (szSender != NULL && strlen(szSender) > 0) //print the sender's name
	{
		...
		cr.cpMax = nLen; //strlen(m_szTextBoxBuffer);	 // Select the last character to make the text insertion
		cr.cpMin  = cr.cpMax;
	...
	case WM_INITDIALOG:
		{
			...
			_snprintf_s(szTitle,lTitleBufSize,lTitleBufSize-1, sz_ID_CHAT_WITH_S_ULTRAVNC,_this->m_szRemoteName);
			...
			// localize
			SetDlgItemText(hWnd, IDC_SEND_B, sz_IDS_SEND41063); 
			SetDlgItemText(hWnd, IDC_HIDE_B, sz_IDS_MINIMIZE41064); 
			SetDlgItemText(hWnd, IDCANCEL, sz_IDS_CLOSE41065); 
