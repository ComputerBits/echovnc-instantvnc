...
public:
	...
	void CaptureScreenFromVideoDriver(const rfb::Rect &rect, BYTE *scrBuff, const BYTE *srcbuffpos);
	int ScreenBuffSize();
	HWND Window() {return m_hwnd;};

	// Mouse related
	void CaptureMouse(BYTE *scrBuff, UINT scrBuffSize);
	void CaptureMouseFromVideoDriver(const rfb::Rect &rect, BYTE *scrBuff, const BYTE *srcbuffpos);
	rfb::Rect MouseRect();
	...
	vncVideoDriver* VideoDriver() const { return m_videodriver; }; 
	vncServer* VncServer() const { return m_server; };

	// Implementation
protected:

	// Routines to hook and unhook us
	BOOL Startup(bool display_changed = false);
	BOOL Shutdown(bool display_changed = false);
