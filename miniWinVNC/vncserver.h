...
//	vncServer();
	vncServer(UINT portRFB=0, UINT portHTTP=0);	//#IR:100303: * : modified constructor of the vncServer
	...
	virtual void KillClient(vncClientId client, BOOL bSendCloseMsg);
	virtual void KillClient(LPSTR szClientName, BOOL bSendCloseMsg); // sf@2002
	virtual void TextChatClient(LPSTR szClientName); // sf@2002

	virtual UINT AuthClientCount();
	virtual UINT UnauthClientCount();

	virtual void KillAuthClients(BOOL bSendCloseMsg);
	virtual void ListAuthClients(HWND hListBox);
	virtual void WaitUntilAuthEmpty();

	virtual void KillUnauthClients(BOOL bSendCloseMsg);
...
public:
	...
	*/
	...
	virtual int SockConnect(BOOL on);
	void ShutDown();
...
*/
	...
	virtual void AutoReconnectId(char* szId){strcpy_s(m_szAutoReconnectId, 64, szId);}
...
//	CDSMPlugin *m_pDSMPlugin;
	...
	char m_szAutoReconnectId[_MAX_PATH];
