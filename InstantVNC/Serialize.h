...
public:
	...
	static UINT GetSize(const char* sz)
	{
		return (strlen(sz) + 1 );
	}

	static UINT GetSizeBinaryData(DWORD dwSize)
	{
		return (sizeof(dwSize) + dwSize );
	}
	...
	bool LoadFromBuffer( BYTE** pData, DWORD& rDwSize)
	{
		DWORD dwSize;
		DWORD dwSizeOfDword = sizeof(dwSize);
		if (m_iSize >= (int) dwSizeOfDword)
		{
			memcpy( &dwSize, m_pBuf + m_iOffset, dwSizeOfDword );
			if (dwSize && m_iSize >= (int) (dwSizeOfDword + dwSize) )
			{
				try
				{
				...
				memcpy( *pData, m_pBuf + m_iOffset + dwSizeOfDword, dwSize );
			}
			rDwSize = dwSize;
			int iLen = dwSize + sizeof(dwSize);
			m_iSize -= iLen;
			m_iOffset += iLen;
			return true;
		}
	...
	bool SaveToBuffer(const BYTE* pData, DWORD dwSize)
	{
		int iLen = sizeof(dwSize) + dwSize;
		if ( m_iSize >= iLen)
		{
			memcpy( m_pBuf + m_iOffset, &dwSize, sizeof(dwSize) );
			memcpy( m_pBuf + m_iOffset + sizeof(dwSize), pData, dwSize );
			m_iSize -= iLen;
			m_iOffset += iLen;
			return true;
		}
